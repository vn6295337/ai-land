...
const AiModelsVisualization_working_version = () => {
  ...
  const fetchModelData = async () => {
    try {
      console.log('Fetching model data from ai_models_main_working_version...');
      const response = await supabase
        .from('ai_models_main_working_version')
        .select('*')
      ...
      if (!response.data || response.data.length === 0) {
        ...
        throw new Error('No data available from ai_models_main_working_version table');
      }
  ...
  if (loading) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className={`text-lg ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Loading AI models data from working version...</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col">
        ...
        <div className="text-center mb-4 relative">
          ...
          <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            AI Models Dashboard <span className="text-lg font-normal">v2.4-TEST</span>
          </h1>
        </div>
        ...
        <div className={`mt-6 p-4 rounded-lg border-2 ${
          ...
          <p className={`text-sm ${darkMode ? 'text-yellow-200' : 'text-yellow-800'}`}>
            🧪 <strong>v2.4-TEST:</strong> Testing new OpenRouter models (23 added). Using ai_models_main_working_version table.
            <br/>
            <strong>New models include:</strong> OpenAI gpt-oss-20b, Google Gemma variants, Meta Llama 3.1-3.3, DeepSeek R1 variants, 
            Qwen 2.5/3.x models, and Moonshot Kimi models.
            <br/>
...
export default AiModelsVisualization_working_version;